...
class SQSDriver extends Driver {
  ...
  parse(context, input) {
    ...
    return messages.reduce((acc, message) => {
      ...
      // To be backwards compatible only apply these values when passed in the message.
      const extraParams = {};

      if (message.dedupId) {
        extraParams.dedupId = message.dedupId;
      }
      if (message.groupId) {
        extraParams.groupId = message.groupId;
      }

      (body.Records || [body]).forEach(record => {
        acc.push(Object.assign({}, message, { body: record, messageAttributes, ...extraParams }));
      });
      return acc;
    }, []);
  }
